// ── NAV active state + scroll reveal ──
  ...
  const I18N = {
    ...
    ja: {
      ...
      's04.jA.s3': '<span class="stat-hi">Checkpoint ①</span>, 分析タイプの単一選択パネル。選ぶか、自分で入力するか。',
      's04.jA.s4': '<span class="stat-hi">Checkpoint ②</span>, 期間とセグメンテーションの複数選択。',
      ...
      's04.jB.body': 'Agent のタスクが数分単位で走るようになると、以前 SQL を見るために Ad-hoc を開いていた 62.5% の人たちには、Agent の内側に監査の導線が必要になります。Journey B が届けるのは <span class="stat-hi">監査レベルの透明性</span>、スピナー → チェックマークで進むストリーミングの思考軌跡、どのステップもタップすれば生クエリや推論へと展開します。最終的な答えには <span class="stat-hi">Receipt</span> が添えられ、すべての判断と、それを支えた SQL を、たためる軌跡として残します。会話を離れずに、そのままデバッグ。',
      ...
      's04.jB.s3': '<span class="stat-hi">インライン Checkpoint</span>, Agent がその場で 1 つだけ確認（「顧客の定義はどちら？」）。ユーザーはチャット内でそのまま返せます。パネルは出ません。',
      's04.jB.s4': '<span class="stat-hi">展開できる原語</span>, どの思考ステップも、SQL・ツール呼び出し・推論チェーンへ展開可能。モードを切り替えずにデバッグ完結。',
      's04.jB.s5': '最終レポートは <span class="stat-hi">Receipt</span> 付きでストリーム。折りたためる SQL と判断の軌跡で、会話を離れずに検証できます。',
    ...
    'zh-tw': {
      ...
      's04.jA.s3': '<span class="stat-hi">Checkpoint ①</span>, 分析類型的單選面板。用戶選一個，或自己打字。',
      's04.jA.s4': '<span class="stat-hi">Checkpoint ②</span>, 時間範圍與分群方式的多選。',
      's04.jA.s5': 'Agent 組 SQL、執行，把分析報告一個字一個字串流出來。',
      's04.jB.eyebrow': '強化 Thinking Steps',
      's04.jB.title': '嚴格稽核員的控制權被繼承，以透明軌跡的形式，不再以模式存在。',
      's04.jB.body': 'Agent 任務跑上數分鐘時，那 62.5% 以前為了看 SQL 而開 Ad-hoc 的人，需要在 Agent 內部有一條稽核路徑。Journey B 給他們 <span class="stat-hi">稽核級的透明度</span>：一條思考原語的串流軌跡（spinner → checkmark），每一步都可展開成背後的 raw SQL 或推理。最終答案會附上 <span class="stat-hi">Receipt</span>，一份可摺疊的決策軌跡與對應 SQL。不離開對話也能除錯。',
      ...
      's04.jB.s3': '<span class="stat-hi">Inline checkpoint</span>, Agent 在對話中問一個澄清問題（「你說的客戶是哪種定義？」）。用戶在聊天中直接回答，不用面板。',
      's04.jB.s4': '<span class="stat-hi">可展開的原語</span>，任何思考步驟都能展開成 SQL、tool call 或推理鏈。不用切換模式也能 debug。',
      's04.jB.s5': '最終報告串流進來，附帶 <span class="stat-hi">Receipt</span>：可摺疊的 SQL + 決策軌跡，不離開對話就能核驗。',
    ...
    'zh-cn': {
      ...
      's04.jA.s3': '<span class="stat-hi">Checkpoint ①</span>, 分析类型的单选面板。用户选一个，或自己打字。',
      's04.jA.s4': '<span class="stat-hi">Checkpoint ②</span>, 时间范围与分群方式的多选。',
      's04.jA.s5': 'Agent 组 SQL、执行，把分析报告一个字一个字流出来。',
      's04.jB.eyebrow': '强化 Thinking Steps',
      's04.jB.title': '严格稽核员的控制权被继承，以透明轨迹的形式，不再以模式存在。',
      's04.jB.body': 'Agent 任务跑上数分钟时，那 62.5% 以前为了看 SQL 而开 Ad-hoc 的人，需要在 Agent 内部有一条稽核路径。Journey B 给他们 <span class="stat-hi">稽核级的透明度</span>：一条思考原语的流轨迹（spinner → checkmark），每一步都可展开成背后的 raw SQL 或推理。最终答案会附上 <span class="stat-hi">Receipt</span>，一份可折叠的决策轨迹与对应 SQL。不离开对话也能调试。',
      ...
      's04.jB.s3': '<span class="stat-hi">Inline checkpoint</span>, Agent 在对话中问一个澄清问题（"你说的客户是哪种定义？"）。用户在聊天中直接回答，不用面板。',
      's04.jB.s4': '<span class="stat-hi">可展开的原语</span>，任何思考步骤都能展开成 SQL、tool call 或推理链。不用切换模式也能 debug。',
      's04.jB.s5': '最终报告流式到达，附带 <span class="stat-hi">Receipt</span>：可折叠的 SQL + 决策轨迹，不离开对话就能核验。',
