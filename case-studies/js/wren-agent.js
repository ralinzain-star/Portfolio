...
// ── NAV active state + scroll reveal ──
  ...
  const I18N = {
    ...
    ja: {
      ...
      's06.hard.label': 'このアーキテクチャが難しくしたこと',
      's06.hard.body': 'いまやすべての機能が同じ入力欄の裏にあります。つまり、何ひとつ自ら名乗り出てきません。旧来の画面は辿りにくかったものの、十数個のボタンは少なくとも「この製品に何ができるか」を伝えていました。そのボタンがまだ画面にあった時点で、トライアルユーザーはすでに <span class="stat-hi-blue">「what can I ask?」</span>と打ち込んでいたのです。ボタンを取り去ってもこの問いには答えられず、むしろ鋭くなります。今その重さを部分的に担っているものが二つあります。Knowledge ページが入口ではなく監査面として残ったこと、そして Agent が回答時に適用したルールを明示することで、事後にシステムが読み解けること。ただしどちらも「機能を見つける」仕組みではありません。ケイパビリティの発見可能性は、このアーキテクチャが背負った負債であり、私が次に取り組む問題です。',
    },
    'zh-tw': {
      'nav.back': '← Iris Hsieh',
      'nav.intro': '.簡介',
      ...
      's06.hard.label': '這個架構讓什麼變難了',
      's06.hard.body': '現在每個能力都待在同一個輸入框後面，也就是說，沒有任何東西會自己現身。舊介面確實難導覽，但那十幾顆按鈕至少告訴了你這個產品做得到什麼。按鈕都還在的時候，試用者就已經在打 <span class="stat-hi-blue">「what can I ask?」</span> 了；把按鈕拿掉並不會回答這個問題，只會讓它更尖銳。今天有兩件事分擔了一部分：Knowledge 頁面沒有消失，它從入口變成了稽核介面；Agent 回答時會指名自己套用了哪一條規則，讓系統在事後仍然讀得懂。但這兩者都不是「發現能力」的機制。能力的可發現性，是這個架構欠下的債，也是我接下來會處理的問題。',
    },
    'zh-cn': {
      'nav.back': '← Iris Hsieh',
      'nav.intro': '.简介',
      ...
      's06.hard.label': '这个架构让什么变难了',
      's06.hard.body': '现在每个能力都待在同一个输入框后面，也就是说，没有任何东西会自己现身。旧界面确实难导览，但那十几颗按钮至少告诉了你这个产品做得到什么。按钮都还在的时候，试用者就已经在打 <span class="stat-hi-blue">「what can I ask?」</span> 了；把按钮拿掉并不会回答这个问题，只会让它更尖锐。今天有两件事分担了一部分：Knowledge 页面没有消失，它从入口变成了审计界面；Agent 回答时会指名自己套用了哪一条规则，让系统在事后仍然读得懂。但这两者都不是「发现能力」的机制。能力的可发现性，是这个架构欠下的债，也是我接下来会处理的问题。',
    }
  };
