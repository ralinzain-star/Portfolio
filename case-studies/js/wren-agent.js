...
// ── NAV active state + scroll reveal ──
  ...
  const I18N = {
    ...
    ja: {
      ...
      's04.replay': 'リプレイ',
      's04.jC.userJourney': 'ユーザージャーニー',
      's04.jC.jUpload': 'Skill をアップロード',
      's04.jC.jTry': '試してみる',
      's04.jC.play': '再生',
      ...
      's05.out1.body': 'ゴールを言葉にするだけ。ローンチから 1 か月以内に、旧入口の利用は月数百件から<span class="stat-hi">1 桁</span>に落ち、誰も戻りませんでした。初期フィードバックは約 <span class="stat-hi">4:1</span> でポジティブ（サンプルは小さく、追跡継続中）。',
      ...
      's01.f4.label': 'AI に教えるための Knowledge も、いちばん使われていない機能のひとつだった',
      's01.f4.body': 'Knowledge は、Wren が正しく答えられるように、チームが指標や定義、業務ルールを書き込んでおくページです。実際に作ったチームは約 <span class="stat-hi">10%</span> にとどまり、採用率は一年を通じて下がり続けました。この時点では、誰も開かない入口がもうひとつある、というだけに見えていました。',
      's02.title': 'インサイト：ユーザーは入口を覚えられない。でも、やりたいことは言える',
      's02.intro': '機能がどこにあるかは指させないのに、何がしたいかははっきり言えました。だから、いつでも見つけられる唯一の入力欄に、それを打ち込んでいたのです。',
      ...
      's03.f1.label': 'Knowledge を作ることは、チームが課金する最も強いシグナルのひとつだった',
      's03.f1.body': 'アクティベーションのデータでは、AI に教えることは、観測できた中でも最も強い課金シグナルのひとつでした。Knowledge を作ったユーザーの課金率は <span class="stat-hi">0.7%</span>、作らなかったユーザーは <span class="stat-hi">0.1%</span>。<span class="stat-hi">約 7 倍</span>です。教えることは「あれば嬉しい」ではありませんでした。測れるものの中で、顧客が取りうる最も価値の高い行動のひとつだったのです。',
      's03.f2.label': 'それなのに、最後までやり切られない機能でもあった',
      's03.f2.body': 'つまり、ほとんど誰もやり切らなかったこの機能こそが、いちばん強いレバーであり、しかも触られないまま置かれていたということです。理由は、そこに辿り着くのに一行ずつ入力する手作業の設定ページが必要で、多くのチームが途中で諦めていたからでした。',
      's03.chart.cap': '課金率（ユーザー単位 · Knowledge を作った人とそれ以外）',
      's03.chart.r1': 'Knowledge を作った',
      's03.chart.r2': '作らなかった',
      's03.chart.tag': '約 7 倍',
      ...
      's03.after.label': 'After · Knowledge は、フォームではなく会話になった',
      's03.after.body': '今は、同僚にブリーフィングするように、話しかけて AI に教えられます。「アクティブ顧客とは、キャンセルされていない注文が 1 件以上あること」。agent は聞き取った内容を選択式の質問で確認し、ルールを markdown ファイルとして下書きし、どうするかを尋ねます。明示的な承認なしには何も公開されず、公開されれば、チーム全体が同じ定義を受け継ぎます。',
      's03.demo.label': '出荷されたフロー、4 つの瞬間',
      's03.demoA.cap': '<b>01 · 推測せず、収束させる。</b>agent は曖昧な依頼を選択式の質問に変えます。「active customer」は、モデルが勝手に想定した定義ではなく、ユーザー自身が選んだひとつの定義に着地します。',
      's03.demoB.cap': '<b>02 · 保存の前に、確認。</b>agent はルールを markdown ファイルとして下書きし、サマリーを見せて、どうするかを尋ねます。「Publish to project knowledge」を選んだときだけ書き戻される。システムは決めつけず、必ず尋ねます。',
      's03.kb.cap': '<b>03 · ページは消えたのではなく、監査面になった。</b>教えられたルールはすべて、バージョン管理できる markdown ファイルとして glossary・rules・SQL パターンに整理され、チーム全員がレビューできます。',
      's03.applied.cap': '<b>04 · ループが閉じる。</b>新しいスレッド、あとから来た質問。agent は答える前に、適用しているルールの名前を出します。<i>using saved definition: active customer = a customer with at least one order where order_status &lt;&gt; \'canceled\'</i>。ひとりがついでに一度教えただけです。それ以降に聞いた人は全員、同じ定義を受け取り、どの定義が使われたかも見えます。',
      's03.rej.label': '途中で捨てた 2 つの設計案',
      's03.rej1.label': 'Knowledge ページを、より賢いフォームに作り直す',
      's03.rej1.body': 'いちばん自然な第一手でした。テンプレート、一括インポート、親切な空状態。しかし 90 日間のデータがこの案を殺しました。ユーザーはページの中で失敗していたのではなく、そもそも辿り着いていなかったのです。誰も訪れない目的地を磨いても、「不在」が生んでいる数字は動きません。',
      's03.rej2.label': 'agent が推測した内容を、黙って保存させる',
      's03.rej2.body': 'いちばん摩擦のないバージョンは、確認せずに修正を取り込むことです。私はこれを却下しました。Knowledge は共有インフラだからです。聞き間違えたひとつの文が、誰にも気づかれないまま、チーム全員の回答を静かに汚染してしまう。下書きと明示的な「Publish to project knowledge」のゲートは、クリック 1 回と引き換えに監査可能性を手に入れる取引です。',
      's03.close': 'これがパターンを証明したケースでした。製品の中でいちばん構造的で、いちばん「設定っぽい」タスクである「教える」ことさえ会話に畳み込めるなら、ほかのすべても同じようにできるはずです。',
      's05.hind.label': 'ふりかえり：いま作り直すなら、何を変えるか',
      's05.hind.li1': '私は revamp に飛びつくのが早すぎました。データは Knowledge が「使われていないのに価値が高い」と示していて、私はすぐにあのページの作り直しへ向かいました。その時間を先に、もっと広いユーザーリサーチと探索に使っていれば、答えにはもっと早く辿り着けたはずです。質問ボックスはすでにユーザーが働いている場所であり、Knowledge はそこで、対話によって組み立てられる。最終的に正しい場所には着きました。ただ遠回りをしただけで、その寄り道は、払わなくてよかった数週間でした。',
    },
    'zh-tw': {
      'nav.back': '← Iris Hsieh',
      'nav.intro': '.簡介',
      ...
      's04.replay': '重播',
      's04.jC.userJourney': '用戶旅程',
      's04.jC.jUpload': '上傳 Skill',
      's04.jC.jTry': '實際試用',
      's04.jC.play': '播放',
      ...
      's05.out1.body': '描述目標就好。上線一個月內，舊入口的使用量從每月數百次掉到<span class="stat-hi">個位數</span>，沒有人回頭；早期回饋的讚與倒讚比例約為 <span class="stat-hi">4:1</span>（樣本尚小，持續追蹤中）。',
      ...
      's01.f4.label': '用來教 AI 的 Knowledge，也是最少人用的功能之一',
      's01.f4.body': 'Knowledge 是一個頁面，團隊在上面寫下自己的指標、定義和業務規則，Wren 才答得準。真正建立過的團隊只有約 <span class="stat-hi">10%</span>，而且採用率整年一路下滑。在這個時間點，它看起來就只是「又一個沒人打開的入口」。',
      's02.title': '洞察：用戶記不住入口，但清楚自己想做什麼',
      's02.intro': '他們指不出功能在哪，但講得出自己想要什麼。於是他們就打進那個永遠找得到的框裡。',
      ...
      's03.f1.body': '在我們的啟用數據裡，教 AI 這件事，是我們觀測得到最強的付費訊號之一。有建立 Knowledge 的用戶付費率是 <span class="stat-hi">0.7%</span>，沒有建立的則是 <span class="stat-hi">0.1%</span>，大約<span class="stat-hi">高出 7 倍</span>。教 AI 不是「有更好」，在所有量得到的行為裡，它是顧客能做的最有價值的事情之一。',
      's03.f2.label': '偏偏，它也是最少被做完的功能之一',
      's03.f2.body': '也就是說，那個幾乎沒人做完的功能，正是我們手上最強的槓桿，而它一直沒被碰過。因為要用到它，得先走完一個要一行一行填寫的手動設定頁，很多團隊做到一半就放棄了。',
      's03.chart.cap': '付費率（以人計 · 有建立 Knowledge 與其餘）',
      's03.chart.r1': '有建立 Knowledge',
      's03.chart.r2': '沒有建立',
      's03.chart.tag': '約 7 倍',
      ...
      's03.after.label': 'After · Knowledge 是一段對話，不是一張表單',
      's03.after.body': '現在你用說的教 AI，就像跟同事交代事情：「活躍顧客的定義，是至少有一筆未取消的訂單。」agent 會用選擇題跟你確認它聽到的內容，把規則起草成一個 markdown 檔案，再問你要怎麼處理。沒有明確的同意，什麼都不會被發佈；一旦發佈，整個團隊繼承的就是同一個定義。',
      's03.demo.label': '實際上線的流程，四個時刻',
      's03.demoA.cap': '<b>01 · 收斂，而不是猜。</b>agent 把模糊的請求變成選擇題，讓「active customer」落在用戶親自選的那一個定義上，而不是模型自行假設的版本。',
      's03.demoB.cap': '<b>02 · 先確認，才儲存。</b>agent 把規則起草成 markdown 檔案、給出摘要，然後問你要怎麼處理。只有按下「Publish to project knowledge」才會寫回。系統從不假設，它永遠先問。',
      's03.kb.cap': '<b>03 · 頁面沒有消失，它變成了稽核介面。</b>每一條教過的規則都是一個可版本控管的 markdown 檔案，分類在 glossary、rules 與 SQL patterns 之下，整個團隊都能檢視。',
      's03.applied.cap': '<b>04 · 迴圈收合。</b>換一條新的對話、換一個之後才問的問題，agent 在回答之前先把它套用的規則講出來：<i>using saved definition: active customer = a customer with at least one order where order_status &lt;&gt; \'canceled\'</i>。這條規則只有一個人、在某次對話裡順手教過一次。之後每一個問的人，拿到的都是同一個定義，而且看得到它用的是哪一條。',
      's03.rej.label': '一路上被我否決的兩個設計',
      's03.rej1.label': '把 Knowledge 頁面重做成一張更聰明的表單',
      's03.rej1.body': '最直覺的第一步：更好的範本、批次匯入、更友善的空狀態。但 90 天的數據否決了它。用戶不是在頁面裡失敗，而是根本沒有抵達。把一個沒人造訪的目的地打磨得再亮，也動不了一個由「缺席」造成的數字。',
      's03.rej2.label': '讓 agent 把它推斷到的東西默默存起來',
      's03.rej2.body': '摩擦最小的版本，是不問就把修正收下。我否決了它，因為 Knowledge 是共享的基礎設施：一句聽錯的話，會在沒有人知道原因的情況下，安靜地污染每個隊友的答案。「草稿＋明確按下 Publish to project knowledge」這道關卡，是用一次點擊換來可稽核性。',
      's03.close': '這就是證明整套模式的那個案例。如果連「教」這件產品裡最結構化、最「像在做設定」的任務，都能收進一段對話，那其他所有事也一樣可以。',
      's05.hind.label': '事後看：如果今天重來，我會改什麼',
      's05.hind.li1': '我太早跳進 revamp。數據告訴我 Knowledge 沒人用、但價值很高，我就直接開始重做那個頁面。如果我先把那段時間花在更廣泛的用戶研究和探索上，我會更早走到答案：輸入框本來就是用戶待著的地方，而 Knowledge 可以就在那裡，用對話建立起來。我最後到了對的地方，只是繞了遠路，而那段彎路花掉的幾個星期，其實不必花。',
    },
    'zh-cn': {
      'nav.back': '← Iris Hsieh',
      'nav.intro': '.简介',
      ...
      's04.replay': '重播',
      's04.jC.userJourney': '用户旅程',
      's04.jC.jUpload': '上传 Skill',
      's04.jC.jTry': '实际试用',
      's04.jC.play': '播放',
      ...
      's05.out1.body': '描述目标就好。上线一个月内，旧入口的使用量从每月数百次掉到<span class="stat-hi">个位数</span>，没有人回头；早期反馈的赞与踩比例约为 <span class="stat-hi">4:1</span>（样本尚小，持续跟踪中）。',
      ...
      's01.f4.label': '用来教 AI 的 Knowledge，也是最少人用的功能之一',
      's01.f4.body': 'Knowledge 是一个页面，团队在上面写下自己的指标、定义和业务规则，Wren 才答得准。真正建立过的团队只有约 <span class="stat-hi">10%</span>，而且采用率整年一路下滑。在这个时间点，它看起来就只是「又一个没人打开的入口」。',
      's02.title': '洞察：用户记不住入口，但清楚自己想做什么',
      's02.intro': '他们指不出功能在哪，但讲得出自己想要什么。于是他们就打进那个永远找得到的框里。',
      ...
      's03.f1.body': '在我们的激活数据里，教 AI 这件事，是我们观测得到最强的付费信号之一。有建立 Knowledge 的用户付费率是 <span class="stat-hi">0.7%</span>，没有建立的则是 <span class="stat-hi">0.1%</span>，大约<span class="stat-hi">高出 7 倍</span>。教 AI 不是「有更好」，在所有量得到的行为里，它是客户能做的最有价值的事情之一。',
      's03.f2.label': '偏偏，它也是最少被做完的功能之一',
      's03.f2.body': '也就是说，那个几乎没人做完的功能，正是我们手上最强的杠杆，而它一直没被碰过。因为要用到它，得先走完一个要一行一行填写的手动设置页，很多团队做到一半就放弃了。',
      's03.chart.cap': '付费率（以人计 · 有建立 Knowledge 与其余）',
      's03.chart.r1': '有建立 Knowledge',
      's03.chart.r2': '没有建立',
      's03.chart.tag': '约 7 倍',
      ...
      's03.after.label': 'After · Knowledge 是一段对话，不是一张表单',
      's03.after.body': '现在你用说的教 AI，就像跟同事交代事情：「活跃客户的定义，是至少有一笔未取消的订单。」agent 会用选择题跟你确认它听到的内容，把规则起草成一个 markdown 文件，再问你要怎么处理。没有明确的同意，什么都不会被发布；一旦发布，整个团队继承的就是同一个定义。',
      's03.demo.label': '实际上线的流程，四个时刻',
      's03.demoA.cap': '<b>01 · 收敛，而不是猜。</b>agent 把模糊的请求变成选择题，让「active customer」落在用户亲自选的那一个定义上，而不是模型自行假设的版本。',
      's03.demoB.cap': '<b>02 · 先确认，才保存。</b>agent 把规则起草成 markdown 文件、给出摘要，然后问你要怎么处理。只有按下「Publish to project knowledge」才会写回。系统从不假设，它永远先问。',
      's03.kb.cap': '<b>03 · 页面没有消失，它变成了审计界面。</b>每一条教过的规则都是一个可版本管理的 markdown 文件，分类在 glossary、rules 与 SQL patterns 之下，整个团队都能查看。',
      's03.applied.cap': '<b>04 · 回路闭合。</b>换一条新的对话、换一个之后才问的问题，agent 在回答之前先把它套用的规则讲出来：<i>using saved definition: active customer = a customer with at least one order where order_status &lt;&gt; \'canceled\'</i>。这条规则只有一个人、在某次对话里顺手教过一次。之后每一个问的人，拿到的都是同一个定义，而且看得到它用的是哪一条。',
      's03.rej.label': '一路上被我否决的两个设计',
      's03.rej1.label': '把 Knowledge 页面重做成一张更聪明的表单',
      's03.rej1.body': '最直觉的第一步：更好的模板、批量导入、更友善的空状态。但 90 天的数据否决了它。用户不是在页面里失败，而是根本没有抵达。把一个没人访问的目的地打磨得再亮，也动不了一个由「缺席」造成的数字。',
      's03.rej2.label': '让 agent 把它推断到的东西默默存起来',
      's03.rej2.body': '摩擦最小的版本，是不问就把修正收下。我否决了它，因为 Knowledge 是共享的基础设施：一句听错的话，会在没有人知道原因的情况下，安静地污染每个队友的答案。「草稿＋明确按下 Publish to project knowledge」这道关卡，是用一次点击换来可审计性。',
      's03.close': '这就是证明整套模式的那个案例。如果连「教」这件产品里最结构化、最「像在做设置」的任务，都能收进一段对话，那其他所有事也一样可以。',
      's05.hind.label': '事后看：如果今天重来，我会改什么',
      's05.hind.li1': '我太早跳进 revamp。数据告诉我 Knowledge 没人用、但价值很高，我就直接开始重做那个页面。如果我先把那段时间花在更广泛的用户研究和探索上，我会更早走到答案：输入框本来就是用户待着的地方，而 Knowledge 可以就在那里，用对话建立起来。我最后到了对的地方，只是绕了远路，而那段弯路花掉的几个星期，其实不必花。',
    }
  };
